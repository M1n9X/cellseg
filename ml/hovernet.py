...
def _make_HoVerNet_residual_block(input_channels, output_channels, stride, n_units):
    ...
    units.append(_HoVerNetResidualUnit(
        input_channels, output_channels, stride))

    for i in range(n_units - 1):
        units.append(_HoVerNetResidualUnit(
            output_channels, output_channels, stride=1))
...
class HoVerNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        outputs = {
            "nucleus_prediction": out_np,
            "horizontal_vertical": out_hv,
        }
        if self.n_classes is not None:
            out_nc = self.nc_branch(encoded)
            out_nc = self.nc_head(out_nc)
            outputs["type_prediction"] = out_nc
...
def _ce_loss_np_head(np_out, true_mask):
    ...
    truth = (
        _convert_multiclass_mask_to_binary(
            true_mask).type(torch.long).squeeze(dim=1)
    )
    ce = nn.CrossEntropyLoss()
...
def loss_hovernet(outputs, ground_truth, n_classes=None):
    ...
    References:
        ...
        Medical Image Analysis, 58, p.101563.
    """
    true_mask, true_hv = ground_truth["nucleus_prediction"], ground_truth["horizontal_vertical"]
