...
from scipy.ndimage import (binary_fill_holes, gaussian_filter,
                           generate_binary_structure, label, measurements)
...
def process(pred_map, return_centroids=False):
    ...
    pred_inst = np.squeeze(np.asarray(pred_map["nucleus_prediction"].cpu()))
    pred_inst = _proc_np_hv(pred_inst,np.squeeze(np.asarray(pred_map["horizontal_vertical"].cpu())))
